          ...
          >
            ...
            <template #cell(all)="data">
              ...
              <div class="pm-row-all">
                ...
                <b-form-checkbox-group class="pm-row-perm" v-model="selected">
                  <template v-for="item in data.item.permissions">
                    <b-form-checkbox
                        v-if="!item.type"
                        :value="item.id"
...
export default {
  ...
  methods: {
    async fetchOneRole() {
      ...
      this.isBusy = true
      await api.roles.fetchOneRole(id)
          .then(res => {
      ...
      this.items.forEach(item => {
        if (item.permissions[0].slug === 'all') {
          ...
          item.permissions[0].id.forEach(item => {
            this.selected.indexOf(item) !== -1 ? checkList.push(true) : checkList.push(false)
          })
          checkList.every(item => item === true) ? item.permissions[0].active = true : item.permissions[0].active = false
        }
      })
    },
