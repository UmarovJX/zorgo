  ...
  <b-row>
    ...
    <b-col cols="12">
      ...
      >
        ...
        <template #cell(policy.number)="data">
          {{ policyNumber(data.item.policy) }}
        </template>
...
export default {
  ...
  methods: {
    ...
    getParams() {
      ...
      if (this.search) {
        params.append(
            'search[id,policy.anketa_id,policy.series,policy.number,' +
    ...
    policyNumber(policy) {
      if (!policy.series) return ''
      return policy.series + policy.number
    },
