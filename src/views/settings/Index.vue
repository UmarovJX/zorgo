        ...
        <ValidationObserver immediate ref="validation-observer">
          <b-row>
            ...
            <b-col md="6" v-for="(data, key) in public_offer">
              <ValidationProvider :name="`Публичная оферта [${key.toUpperCase()}]`" rules="mimes:application/pdf"
                                  v-slot="{errors}">
                <b-form-group
    ...
    update() {
      this.$refs['validation-observer'].validate().then(isValid => {
        if (isValid) {
          ...
          formData.append('social[instagram]', this.item.social.instagram ?? '')
          formData.append('social[youtube]', this.item.social.youtube ?? '')
